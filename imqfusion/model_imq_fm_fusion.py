...
Initializing model:
    ...
    num_segments:       {}.
        ...
        self.conv3d = Simple3dConv(512,512,512)
        self.key_feature_layer = nn.Linear(512, KEY_FEATURES)
        ...
        for features in inputs:
            ...
            for i in range(len(features)):
                if i == 0:
                    ...
                    fm = self.base_model_channel_3((features[i], features[i + 2]))
                if i == 1:
                    # for mv and residual need batch_normalization
                    features[i] = self.data_bn_channel_2(features[i])
                    fm = self.base_model_channel_2(features[i])
                if i == 2:
                    continue
                mix_features.append(fm)
            x = self.motion_guide_apperance_tmc(mix_features[0],mix_features[1],self.conv1x1_channel_wise,self.conv1x1_spatial)
            x = self.avgpool(x)
            x = x.flatten(1)
    ...
    def motion_guide_apperance_tmc(self, f_img, f_mv, conv1x1_channel_wise, conv1x1_spatial):
...
class Simple3dConv(nn.Module):
    def __init__(self, input_planes, mid_planes, output_planes):
        super(Simple3dConv, self).__init__()
        self.layer1 = nn.Sequential(
            nn.Conv3d(input_planes, mid_planes, kernel_size=(1,3,3), padding=(0,1,1), stride=1),
            nn.AvgPool3d(kernel_size=(1,3,3), stride=1, padding=(0,1,1)),
            nn.BatchNorm3d(mid_planes),
            nn.ReLU(inplace=True)
        )
        self.layer2 = nn.Sequential(
            nn.Conv3d(mid_planes, output_planes, kernel_size=(1,3,3), padding=(0,1,1), stride=1),
...
class VideoResNetAttention3C(nn.Module):
    def __init__(self, arch='r2plus1d_18', pretrained=True, progress=True,
        ...
        self.attention_layer = Simple3dConv(1, 32,64)
...
class VideoResNetAttention2C(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.layer2(x)
        x = self.layer3(x)
        x = self.layer4(x) # (batch,channel=512,d=2,w=7,h=7)
        #here return a feature map
        return x
