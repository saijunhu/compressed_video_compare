...
int decode_video(
    ...
    AVCodec *pCodec;//co-decoder
    AVCodecContext *pCodecCtx= NULL;  
    AVCodecParserContext *pCodecParserCtx=NULL;  

    FILE *fp_in;
    AVFrame *pFrame;//after decode frame
    AVFrame *pFrameBGR;
    ...
    int cur_gop = -1; //d
    int cur_pos = 0;
    AVPacket packet;  // Packet, compressed data
    int ret, got_picture;
...
//    fp_in = fopen("data/test.txt",'rb');
//    if(!fp_in){ printf("open txt filed\n");}
//    else{ printf("------------fopen is ok\n");}
    fp_in = fopen(filename, "rb");
    if (!fp_in) {
        printf("haha deconding failed----------------%s------\n",filename);
        perror("Error");
        printf("Could not open input stream\n");
        return -1;
    } else{ printf("test is succ\n");}
    ...
    //this 'while' read the video file
    while (1) {
        ...
        while (cur_size>0){  
            ...
            //if gop is our wanted gop,then we need the calcul until the target frame(pos in a gop)
...
// calculate the frames and gops in videos, just call the ffmpeg funcs,
void count_frames(int* gop_count, int* frame_count) {
    ...
    //Input File
    //TODO bug
//    printf("----------------following code test fopen func-------------------");
//    fp_in = fopen("/home/husaijun/Projects/pytorch-coviar/data/IamLegend_walk_f_nm_np1_fr_med_16.mp4","rb");
//    if(!fp_in){
//        printf("open txt failed\n");
//        perror("Error");
//     }else{
//        printf("------------fopen is well-------------\n");
//        fclose(fp_in);
//     }
    fp_in = fopen(filename, "rb");
    if (!fp_in) {
        printf("haha count frames failed----------------%s------\n",filename);
        perror("Error");
        printf("Could not open input stream\n");  
        return -1;  
    } else{ printf("test is succ\n");}
    ...
    *frame_count = 0;
    ...
    while (1) {
        ...
        while (cur_size>0){  
            ...
            if(packet.size==0)  
                continue;
            // calcul the GOP == calcul the I-frame counts
            if (pCodecParserCtx->pict_type == AV_PICTURE_TYPE_I) {
                ++(*gop_count);
            }
            // the total is frame_count
            ++(*frame_count);
        }
    }
...
// Getting number of GOPs in a video.
...
// return a number of frames in a video.
...
static PyObject *load(PyObject *self, PyObject *args)
{
    ...
    PyArrayObject *bgr_arr = NULL; //rgb information
    PyArrayObject *final_bgr_arr = NULL;
    PyArrayObject *mv_arr = NULL; //motion vectors
    PyArrayObject *res_arr = NULL; //residual
    ...
    if(decode_video(gop_target, pos_target,
        ...
        Py_XDECREF(bgr_arr); //Py_XDECREF reduce reference count
        Py_XDECREF(res_arr);
        return Py_None;
    }
    if(representation == MV) {
