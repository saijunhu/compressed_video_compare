...
def color_aug(img, random_h=36, random_l=50, random_s=50):
    ...
    # random.random() return a float [0.1]
...
class GroupRandomHorizontalFlip(object):
    ...
    def __call__(self, img_group, is_mv=False):
        if random.random() < 0.5:
            # reverse the first axis is vertical flip,  reverse the second axis is horizontal flip
            ret = [img[:, ::-1, :].astype(np.int32) for img in img_group]
            if self._is_mv:
                for i in range(len(ret)):
                    ret[i] -= 128
                    ret[i][..., 0] *= (-1)#change the mv direction
                    ret[i] += 128
            return ret
        else:
            return img_group
...
class GroupScale(object):
    ...
    def __call__(self, img_group):
        if img_group[0].shape[2] == 3:# if channel is 3
            return [cv2.resize(img, self._size, cv2.INTER_LINEAR) for img in img_group]
        elif img_group[0].shape[2] == 2:# need extra deal resize mv
...
class GroupMultiScaleCrop(object):
    def __init__(self, input_size, scales=None, max_distort=1, fix_crop=False, more_fix_crop=True):
        #TODO the 875 seem like a bug,should be .875
        self.scales = scales if scales is not None else [1, .875, .75, .66]
        self.max_distort = max_distort
        self.fix_crop = fix_crop
        self.more_fix_crop = more_fix_crop
        # if is a num then extend to [num,num] else [num1,num2]
